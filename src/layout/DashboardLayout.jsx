...
const DashboardLayout = () => {
    ...
    // We check if the token has been saved to localStorage by AuthContext.
    useEffect(() => {
        if (user) {
            ...
            if (token) {
                ...
                // Keep waiting for AuthContext to fetch the token
	...
	useEffect(() => {
		...
		if(user?.email && isTokenReady) {
			...
			// Use /api path for role check fetch and pass the token
    ...
    // This prevents rendering the dashboard structure before roles are determined.
